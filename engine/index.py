...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        start_position, \
        _, \
        length_in_bytes_posting_list, \
        length_in_bytes_tf_list = self.postings_dict[term]

        self.index_file.seek(start_position)
        postings_list_in_encoded_bytes = self.index_file.read(length_in_bytes_posting_list)
        tf_list_in_encoded_bytes = self.index_file.read(length_in_bytes_tf_list)

        postings_list = self.postings_encoding.decode(postings_list_in_encoded_bytes)
        tf_list = self.postings_encoding.decode(tf_list_in_encoded_bytes)
