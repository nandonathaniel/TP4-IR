...
def eval_retrieval(qrels, query_file="qrels-folder/test_queries.txt", k=100, use_letor=False):
    ...
    print(f"RBP score = {sum(rbp_scores_bm25[i]) / len(rbp_scores_bm25[i]):>.2f}")
    print(f"DCG score = {sum(dcg_scores_bm25[i]) / len(dcg_scores_bm25[i]):>.2f}")
    print(f"AP score  = {sum(ap_scores_bm25[i]) / len(ap_scores_bm25[i]):>.2f}")
